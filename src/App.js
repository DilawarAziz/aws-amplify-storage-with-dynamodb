...
function App() {
  ...
  const fetchSongs = async () => {
    try {
      const songData = await API.graphql(graphqlOperation(listTodos));
      const songList = songData.data.listTodos.items;
      setSongs(songList);
    } catch (error) {
  ...
  const addLike = async (idx) => {
    try {
      ...
      const songData = await API.graphql(
        graphqlOperation(updateTodo, { input: song })
      );
